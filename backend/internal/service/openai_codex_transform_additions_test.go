...
// applyCodexClientMetadata：用派生 installation_id，绝不写 openai_device_id 原文。
func TestApplyCodexClientMetadata(t *testing.T) {
	acc := newTestOAuthAccount(1, map[string]any{"openai_device_id": "dev-xyz"})
	want := resolveConvergedInstallationID(acc, testCodexFingerprintSeed)
	require.NotEqual(t, "dev-xyz", want)

	body := map[string]any{}
	require.True(t, applyCodexClientMetadata(body, acc))
	cm, ok := body["client_metadata"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, want, cm["x-codex-installation-id"])
	...
	require.Equal(t, want, cm3["x-codex-installation-id"])
}
