...
func TestCloseOpenAIWSFailoverExhausted_ErrorPassthroughRule(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ginCtx, _ := gin.CreateTestContext(recorder)
		ginCtx.Request = r
		service.BindErrorPassthroughService(ginCtx, epSvc)
		closeOpenAIWSFailoverExhausted(ginCtx, conn, &service.UpstreamFailoverError{
...
func TestCloseOpenAIWSFailoverExhausted_NativeGateKeepsCloseOnly(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	readCtx, cancelRead := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancelRead()
	// Native (patch off): no error event is sent — the first frame is the close.
	_, _, err = clientConn.Read(readCtx)
	var closeErr coderws.CloseError
	require.ErrorAs(t, err, &closeErr)
	require.Equal(t, coderws.StatusTryAgainLater, closeErr.Code)
