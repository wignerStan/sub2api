...
impl HttpClientBuilder {
    ...
    /// Selects the rustls TLS backend (aws-lc-rs) with native-root verification,
    /// matching the official Codex client fingerprint.
    ...
    /// Forces all connections through an explicit outbound proxy URL
    /// (for example `http://127.0.0.1:1080` or `socks5h://user:pass@host:1080`),
    /// bypassing system and environment proxy resolution.
    ///
    /// Added by sub2api sidecar fork; missing from upstream codex.
    pub fn with_proxy(mut self, url: String) -> Self {
        self.manual_proxy = Some(url);
        self
    }
    ...
    fn reqwest_builder(self, proxy_routing: ProxyRouting) -> reqwest::ClientBuilder {
        let has_manual_proxy = self.manual_proxy.is_some();
        let builder = self.base_reqwest_builder();
        if has_manual_proxy {
            // A manual proxy overrides both transport-default and direct routing;
            // reqwest's no_proxy() would otherwise clear the explicitly added proxy.
            return builder;
        }
        match proxy_routing {
