...
impl WebSocketConnector {
    ...
    /// Connects a WebSocket through an explicit outbound proxy URL
    /// (for example `socks5h://user:pass@host:1080` or `http://127.0.0.1:1080`),
    /// bypassing system and environment proxy resolution.
    ///
    /// `proxy_url: None` forces a direct connection.
    ...
    pub async fn connect_via(
        ...
        let proxy_route = match proxy_url {
