...
use crate::mimic::types::{
    ...
    EXPLICITLY_STRIPPED_TRACE_AND_TRACKING_NAMES, REALISTIC_GIT_BRANCHES,
...
fn deterministic_uuid_convergence_with_salt() {
    ...
    assert_eq!(
        id1, id2,
        "same seed & salt must produce identical installation_id"
    );
    ...
    assert_eq!(
        th1, th2,
        "same client session must produce identical thread_id"
    );
}
...
fn window_number_preserves_protocol_fidelity_across_compaction() {
    let identity_init =
        ConvergedIdentity::new("seed_compaction", Some("sess"), None, "salt", None, 0);
    assert_eq!(identity_init.window_number, 0);
    assert!(identity_init.window_id.ends_with(":0"));

    // After compaction advances to window 2
    let identity_compacted =
        ConvergedIdentity::new("seed_compaction", Some("sess"), None, "salt", None, 2);
    assert_eq!(identity_compacted.window_number, 2);
    assert!(identity_compacted.window_id.ends_with(":2"));
    assert_eq!(
        identity_compacted.thread_id, identity_init.thread_id,
        "thread_id remains stable across compaction"
    );
}
...
fn groups_have_distinct_organic_environments() {
    ...
    let env_b = ConvergedIdentity::new(
        "group_finance_infra",
        None,
        None,
        "salt",
...
fn custom_device_id_is_pseudonymized() {
    let identity =
        ConvergedIdentity::new("seed", None, Some("raw_hardware_id_abc"), "salt", None, 0);
...
fn fail_closed_metadata_sanitization_and_mimic() {
    ...
    assert_eq!(
        res,
        Err(MimicError::ForbiddenMetadataKey(
            "unknown_extra_telemetry".to_string()
        ))
    );
...
fn fingerprint_double_check_and_window_consistency() {
    ...
    assert!(sanitize_client_metadata(
        &mut consistent_meta,
        &identity,
        UnknownFieldPolicy::Forbidden
    )
    .is_ok());
    let tm_str = consistent_meta
    ...
    let err_th = sanitize_client_metadata(
        &mut diverging_thread_meta,
        &identity,
        UnknownFieldPolicy::Forbidden,
    );
    assert!(matches!(
    ...
    let err_win = sanitize_client_metadata(
        &mut diverging_win_meta,
        &identity,
        UnknownFieldPolicy::Forbidden,
    );
    assert!(matches!(
    ...
    let err_prev = sanitize_client_metadata(
        &mut diverging_prev_meta,
        &identity,
        UnknownFieldPolicy::Forbidden,
    );
    assert!(matches!(
    ...
    let err_nested = sanitize_client_metadata(
        &mut diverging_nested_meta,
        &identity,
        UnknownFieldPolicy::Forbidden,
    );
    assert!(matches!(
    ...
    let err_missing = sanitize_and_inject_headers(
        &mut missing_headers,
        "test_group_seed",
        Some("sess"),
        None,
    ...
    assert!(matches!(
        err_missing,
        Err(MimicError::ForbiddenMissingFingerprint(_))
    ));
    ...
    div_headers.insert(
        ...
        HeaderValue::from_static("sess_123"),
    );
    div_headers.insert(
        HeaderName::from_static("thread-id"),
        HeaderValue::from_static("wrong_thread"),
    );
    div_headers.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("actual_thread:2"),
    );
    let err_hdr_div = sanitize_and_inject_headers(
        &mut div_headers,
        "test_group_seed",
        Some("sess_123"),
        None,
    ...
    valid_headers.insert(
        HeaderName::from_static("thread-id"),
        HeaderValue::from_static("actual_thread"),
    );
    valid_headers.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("actual_thread:2"),
    );
    assert!(sanitize_and_inject_headers(
        &mut valid_headers,
        "test_group_seed",
        Some("sess_123"),
        None,
    ...
    .is_ok());
    assert_eq!(
        valid_headers.get("originator").unwrap().to_str().unwrap(),
        "codex_cli_rs"
    );
    assert_eq!(
        valid_headers.get("version").unwrap().to_str().unwrap(),
        "0.1.183"
    );
}
...
fn window_id_bug_fix_and_compaction_lifecycle() {
    let identity_w0 = ConvergedIdentity::new(
        "group_seed",
        Some("sess_1"),
        None,
        "salt",
    ...
    let identity_w1 = ConvergedIdentity::new(
        "group_seed",
        Some("sess_1"),
        None,
        "salt",
    ...
    let identity_w2 = ConvergedIdentity::new(
        "group_seed",
        Some("sess_1"),
        None,
        "salt",
    ...
    let identity_w3 = ConvergedIdentity::new(
        "group_seed",
        Some("sess_1"),
        None,
        "salt",
    ...
    assert!(
        sanitize_client_metadata(&mut valid_w0, &identity_w0, UnknownFieldPolicy::Forbidden)
            .is_ok()
    );
    ...
    let err_w0_flat = sanitize_client_metadata(
        &mut bug_w0_flat,
        &identity_w0,
        UnknownFieldPolicy::Forbidden,
    );
    ...
    let err_w0_nested = sanitize_client_metadata(
        &mut bug_w0_nested,
        &identity_w0,
        UnknownFieldPolicy::Forbidden,
    );
    ...
    assert!(
        sanitize_client_metadata(&mut valid_w1, &identity_w1, UnknownFieldPolicy::Forbidden)
            .is_ok()
    );
    ...
    assert!(
        sanitize_client_metadata(&mut valid_w2, &identity_w2, UnknownFieldPolicy::Forbidden)
            .is_ok()
    );
    ...
    assert!(
        sanitize_client_metadata(&mut valid_w3, &identity_w3, UnknownFieldPolicy::Forbidden)
            .is_ok()
    );
    ...
    hdr_w0.insert(
        HeaderName::from_static("session-id"),
        HeaderValue::from_static("sess_1"),
    );
    hdr_w0.insert(
        HeaderName::from_static("thread-id"),
        HeaderValue::from_static("th_abc"),
    );
    hdr_w0.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("th_abc:0"),
    );
    assert!(sanitize_and_inject_headers(
        &mut hdr_w0,
        "group_seed",
    ...
    let err_hdr_w0 = sanitize_and_inject_headers(
        &mut hdr_w0_bad,
        "group_seed",
        Some("sess_1"),
        None,
    ...
    hdr_w2.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("th_abc:2"),
    );
    hdr_w2.insert(
            HeaderName::from_static("x-codex-turn-metadata"),
...
fn subagent_and_parent_thread_consistency() {
    ...
    assert!(
        sanitize_client_metadata(&mut valid_meta, &identity, UnknownFieldPolicy::Forbidden).is_ok()
    );
    ...
    valid_sub_headers.insert(
        HeaderName::from_static("session-id"),
        HeaderValue::from_static("sess_1"),
    );
    valid_sub_headers.insert(
        HeaderName::from_static("thread-id"),
        HeaderValue::from_static("th_child"),
    );
    valid_sub_headers.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("th_child:0"),
    );
    valid_sub_headers.insert(
        HeaderName::from_static("x-codex-parent-thread-id"),
        HeaderValue::from_static("018f3a7e-4b21-7000-8000-112233445566"),
    );
    valid_sub_headers.insert(
    ...
    bad_uuid_headers.insert(
        HeaderName::from_static("x-codex-parent-thread-id"),
        HeaderValue::from_static("bad-uuid"),
    );
    assert!(matches!(
        sanitize_and_inject_headers(
            &mut bad_uuid_headers,
            "group_seed",
            Some("sess_1"),
            None,
            "salt",
...
fn headers_sanitization_preserves_turn_state_and_strips_tracking() {
    ...
    headers.insert(
        ...
        HeaderValue::from_static("oai_session=leaked_cookie"),
    );
    headers.insert(
        HeaderName::from_static("x-oai-attestation"),
        HeaderValue::from_static("attest_token"),
    );
    headers.insert(
        HeaderName::from_static("x-codex-turn-state"),
        HeaderValue::from_static("server_turn_state_token_123"),
    );
    headers.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("client_th:2"),
    );
    headers.insert(
        axum::http::header::USER_AGENT,
        HeaderValue::from_static("OpenAI/Codex/0.1.183 (Unknown 1.0)"),
    );
    ...
    assert_eq!(
        headers.get("x-codex-turn-state").unwrap().to_str().unwrap(),
        "server_turn_state_token_123"
    );
    assert_eq!(
...
fn transform_request_body_roundtrip() {
    ...
    assert_eq!(
        parsed.get("prompt_cache_key").unwrap(),
        "client_session_abc"
    );
    ...
    assert_eq!(
        res,
        Err(MimicError::ForbiddenMetadataKey(
            "unauthorized_field".to_string()
        ))
    );

    // Unknown extra field stripped under Strip policy
    let res_strip = transform_request_body(
        &bad_raw,
        "seed_42",
        None,
        "salt_1",
    ...
    assert!(parsed_strip
        .pointer("/client_metadata/unauthorized_field")
        .is_none());
}
...
fn transform_ws_frame_roundtrip() {
    ...
    let transformed = transform_ws_frame(
        &raw_str,
        "seed_ws",
        None,
        "salt_ws",
    ...
    assert_eq!(
        res,
        Err(MimicError::ForbiddenMetadataKey(
            "bad_telemetry".to_string()
        ))
    );

    // Unknown extra field stripped under Strip policy
    let res_strip = transform_ws_frame(
        &bad_str,
        "seed_ws",
        None,
        "salt_ws",
    ...
    assert!(parsed_strip
        .pointer("/client_metadata/bad_telemetry")
        .is_none());
}
...
fn workspaces_strip_remote_url_and_preserve_sandbox() {
    ...
    assert_eq!(
        sanitized_tm.get("sandbox_mode").unwrap(),
        "danger-full-access"
    );
    ...
    assert_eq!(
        ws_info.get("latest_git_commit_hash").unwrap(),
        "a1b2c3d4e5f67890"
    );
...
fn git_branch_uses_normal_names() {
    for seed_idx in 0..20 {
        let identity =
            ConvergedIdentity::new(&format!("seed_{seed_idx}"), None, None, "salt", None, 0);
        assert!(
...
fn user_agent_and_workspace_path_are_mutually_consistent() {
    for seed_idx in 0..10 {
        ...
        if identity.os == "darwin" {
            assert!(
                ws_path.starts_with("/Users/"),
                "Darwin must produce /Users/ path: {}",
                ws_path
            );
            assert!(
                win_path.starts_with("/Users/"),
                "Darwin must produce /Users/ path from Windows path: {}",
                win_path
            );
            assert!(
                win_path.ends_with("/my_tool"),
                "Windows path must extract project name: {}",
                win_path
            );
        } else {
            assert!(
                ws_path.starts_with("/home/"),
                "Linux must produce /home/ path: {}",
                ws_path
            );
            assert!(
                win_path.starts_with("/home/"),
                "Linux must produce /home/ path from Windows path: {}",
                win_path
            );
            assert!(
                win_path.ends_with("/my_tool"),
        ...
        headers.insert(
            HeaderName::from_static("x-codex-window-id"),
            HeaderValue::from_static("th_1:0"),
        );
        headers.insert(
            axum::http::header::USER_AGENT,
            HeaderValue::from_static("OpenAI/Codex/0.1.183 (Unknown)"),
        );
        assert!(sanitize_and_inject_headers(
            &mut headers,
            &seed,
        ...
        if identity.os == "darwin" {
            assert!(
                ua.contains("Darwin"),
                "Darwin OS must reflect in User-Agent: {}",
                ua
            );
        } else {
            assert!(
                ua.contains("Linux"),
                "Linux OS must reflect in User-Agent: {}",
                ua
            );
        }
    }
...
fn account_vs_inference_header_fail_closed() {
    ...
    account_headers.insert(
        ...
        HeaderValue::from_static("acc_org_123"),
    );
    account_headers.insert(
    ...
    account_headers.insert(
        HeaderName::from_static("traceparent"),
        HeaderValue::from_static("00-trace-01"),
    );
    account_headers.insert(
        HeaderName::from_static("x-oai-attestation"),
        HeaderValue::from_static("attest_token"),
    ...
    bad_account_headers.insert(
        HeaderName::from_static("x-custom-leak"),
        HeaderValue::from_static("drop_me"),
    );
    let err = sanitize_and_inject_headers(
        &mut bad_account_headers,
        "seed",
        None,
        None,
        "salt",
    ...
    assert_eq!(
        err,
        Err(MimicError::ForbiddenHeader("x-custom-leak".to_string()))
    );

    // Unknown extra x- header on account path under Strip policy -> Ok(()) and stripped
    let mut strip_account_headers = HeaderMap::new();
    strip_account_headers.insert(
        HeaderName::from_static("x-custom-leak"),
        HeaderValue::from_static("drop_me"),
    );
    let res_strip = sanitize_and_inject_headers(
        &mut strip_account_headers,
        "seed",
        None,
        None,
        "salt",
    ...
    inference_headers.insert(
        HeaderName::from_static("thread-id"),
        HeaderValue::from_static("thread_123"),
    );
    inference_headers.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("thread_123:0"),
    );
    inference_headers.insert(
        HeaderName::from_static("x-codex-turn-state"),
        HeaderValue::from_static("state_token"),
    );
    inference_headers.insert(
        HeaderName::from_static("traceparent"),
        HeaderValue::from_static("00-trace-01"),
    ...
    bad_inference_headers.insert(
        HeaderName::from_static("x-custom-leak"),
        HeaderValue::from_static("drop_me"),
    );
    let err2 = sanitize_and_inject_headers(
        &mut bad_inference_headers,
        "seed",
        None,
        None,
        "salt",
    ...
    strip_inference_headers.insert(
        HeaderName::from_static("session-id"),
        HeaderValue::from_static("sess_123"),
    );
    strip_inference_headers.insert(
        HeaderName::from_static("x-codex-window-id"),
        HeaderValue::from_static("th_123:0"),
    );
    strip_inference_headers.insert(
        HeaderName::from_static("x-custom-leak"),
        HeaderValue::from_static("drop_me"),
    );
    let res_strip2 = sanitize_and_inject_headers(
        &mut strip_inference_headers,
        "seed",
        None,
        None,
        "salt",
    ...
    let json_err = sanitize_and_inject_headers(
        &mut json_accept_headers,
        "seed",
        None,
        None,
        "salt",
    ...
    assert_eq!(
        json_err,
        Err(MimicError::ForbiddenAcceptHeader(
    ...
    .is_ok());
    assert_eq!(
        sse_accept_headers.get("accept").unwrap().to_str().unwrap(),
        "text/event-stream"
    );
}
...
fn upstream_codex_wire_snapshot_exact_fidelity() {
    ...
    let snapshot: Value =
        serde_json::from_str(snapshot_raw).expect("valid codex_wire_snapshot.json");
    ...
    test_headers.insert(
        HeaderName::from_static("x-future-upstream-unresolved-header"),
        HeaderValue::from_static("drop"),
    );
    let hdr_res = sanitize_and_inject_headers(
        &mut test_headers,
        "seed",
        None,
        None,
    ...
    assert_eq!(
        hdr_res,
        Err(MimicError::ForbiddenHeader(
            "x-future-upstream-unresolved-header".to_string()
        ))
    );
    ...
    let cm_res = sanitize_client_metadata(
        test_cm.get_mut("client_metadata").unwrap(),
        &identity,
        UnknownFieldPolicy::Forbidden,
    );
    assert_eq!(
        cm_res,
        Err(MimicError::ForbiddenMetadataKey(
            "unrecognized_key_123".to_string()
        ))
    );
}
